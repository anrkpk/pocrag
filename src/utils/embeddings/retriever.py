...
def create_retriever(vector_store, search_k=5, embedding_model_name= "all-MiniLM-L6-v2"):
    """
    Create a retriever from the given vector store.
    
    Args:
        vector_store (FAISS): The FAISS vector store to create a retriever from.
        search_k (int): The number of documents to retrieve.
        
    Returns:
        FAISS: The created retriever.
    """
    try:
